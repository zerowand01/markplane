...
The existing `extract_references()` function and `QueryFilter` infrastructure provide the building blocks. Per-item context (TASK-haky9) is the most impactful — it's what an AI needs when starting work on a specific task. Domain filtering (TASK-3c585) builds on QueryFilter. Clipboard output (TASK-rznmc) is trivial but enables workflows with AI tools that don't support MCP.
