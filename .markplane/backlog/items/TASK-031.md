...
The first migration (v1→v2) will be the sequential→random ID conversion from [[TASK-030]]. The framework should be built to make adding future migrations straightforward — each migration is a function with a clear signature that transforms `.markplane/` data from version N to N+1.
