...
- **MCP module** (`src/mcp/`): The `markplane mcp` subcommand runs the MCP server enabling AI tools (Claude, Cursor, etc.) to interact with the project
  ...
  - **Tools**: 17 tools — `markplane_summary`, `markplane_context`, `markplane_query`, `markplane_show`, `markplane_graph`, `markplane_add`, `markplane_update`, `markplane_start`, `markplane_done`, `markplane_promote`, `markplane_plan`, `markplane_link`, `markplane_archive`, `markplane_unarchive`, `markplane_sync`, `markplane_check`, `markplane_stale`
